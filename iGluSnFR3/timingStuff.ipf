...
for (i=0;i<11;i+=1)
	...
	make/o/n=(nStimEvents) tempPhaseTimes, sinPart, cosPart, phaseAngle
	for (j=0;j<nStimEvents;j+=1)
		...
		phaseAngle[j] = 2 * pi * freqs[i] * tempPhaseTimes[j]
	endfor

	vecStren = 1/nStimEvents * sqrt(sum(sinPart)^2 + sum(cosPart)^2)
	tempjitt = sqrt(2 * (1-vecStren))/(2 * pi * freqs[i])
	...
	make/o/n=(nStimEvents) qPhaseTimes, qSinPart, qCosPart, qPhaseAngle
	onWhichQuanta = 0
		for (j=0;j<nStimEvents;j+=1)
			if (stimAmps[j]==k)
				...
				qPhaseAngle[onWhichQuanta] = 2 * pi * freqs[i] * qPhaseTimes[j]
				onWhichQuanta+=1
			endif
		endfor
		...
		qFreqTempJitt[i][k] = sqrt(2*(1-qFreqVecStren[i][k]))/(2 * pi * freqs[i])
...
make/o/n=(nStimEvents) phaseTimes, sinPart, cosPart, phaseAngle
variable i
for (i=0;i<nStiMEvents;i+=1)
	...
	phaseAngle[i] = 2 * pi * freq * phaseTimes[i]
endfor


variable vecStren = 1/nStimEvents  * sqrt(sum(sinPart)^2 + sum(cosPart)^2)
variable tempJitt = sqrt(2*(1-vecStren))/(2 * pi* freq)
...
for (i=0;i<maxQ+1;i+=1)
	make/o/n=(nStimEvents) qPhase,qSin,qcOS, qPhaseAngle
	onWhichInd = 0
	for (j=0;j<nStimEvents;j+=1)	
		if (stimAmps[j]==i)
			...
			qPhaseAngle[onWhichInd] = 2 * pi * freq * qPhase[onWhichInd]
			onWhichInd+=1
		endif
	endfor
	...
	qTJ[i] = sqrt(2 * (1-qVS[i]))/(2 * pi * freq)
